...
qid_to_question = {f"Q{int(row['Id'])}": str(row["Question"]).strip() for _, row in df_q.iterrows()}
answer_lookup = df_a.set_index(["Set_Id", "Id"])["Answer"].to_dict()
qid_to_answer_set = {
    f"Q{int(row['Id'])}": str(row["Answer Set"])
    for _, row in df_q.iterrows() if not pd.isna(row["Answer Set"])
}
